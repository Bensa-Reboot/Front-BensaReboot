...
class ProductGrid extends HTMLElement {
  ...
  connectedCallback() {
    ...
    this._productPathPrefix = window.location.pathname.includes("/pages/")
      ? ""
      : "pages/";
    ...
    if (tipo === "busca" && busca) {
      ...
      this._allProducts = unique.filter((p) =>
  ...
  _productCardTemplate(p) {
    ...
    <article class="product-card">
      <div class="product-top">
        ...
        <a href="${this._productPathPrefix}produto.html?id=${p.id}" aria-label="Ver produto ${p.title}">
          <img src="${p.image}" alt="${p.title}">
        </a>
      </div>
      ${badge}
      <a class="product-title" href="${this._productPathPrefix}produto.html?id=${p.id}">${p.title}</a>
  ...
  _render() {
    ...
    this.innerHTML = this._filtered
      .map((p) => this._productCardTemplate(p))
      .join("");
  }
  ...
  _dropdown(anchorBtn, items, onSelect) {
    ...
    anchorBtn.addEventListener("click", (e) => {
      ...
      items.forEach((item) => {
        ...
        li.style.cssText =
          "padding:12px 20px;cursor:pointer;font-size:14px;font-weight:600;font-family:Montserrat,sans-serif;white-space:nowrap;color:#151515;";
        li.addEventListener(
          "mouseenter",
  ...
  _bindSort() {
    const btn = [...document.querySelectorAll(".catalog-tools button")].find(
      (b) => b.textContent.includes("Ordenar"),
    );
    if (!btn) return;
    const options = [
      {
        label: "Menor preço",
        action: () => {
          this._filtered = [...this._filtered].sort(
            (a, b) => a.price - b.price,
          );
          this._render();
        },
  ...
  _bindFilter() {
    const btn = [...document.querySelectorAll(".catalog-tools button")].find(
      (b) => b.textContent.includes("Filtrar"),
    );
    if (!btn) return;
    const filters = [
      {
      ...
      },
      {
        label: "Até R$150",
        action: () => {
          this._filtered = this._allProducts.filter((p) => p.price <= 150);
          this._render();
        },
      },
      {
        label: "Até R$300",
        action: () => {
          this._filtered = this._allProducts.filter((p) => p.price <= 300);
          this._render();
        },
      },
      {
        label: "Até R$500",
        action: () => {
          this._filtered = this._allProducts.filter((p) => p.price <= 500);
          this._render();
        },
      },
      {
        label: "Até R$800",
        action: () => {
          this._filtered = this._allProducts.filter((p) => p.price <= 800);
          this._render();
        },
      },
      {
        label: "Acima de R$800",
        action: () => {
          this._filtered = this._allProducts.filter((p) => p.price > 800);
          this._render();
        },
      },
      {
        label: "Best Seller",
        action: () => {
          this._filtered = this._allProducts.filter(
      ...
      },
      {
        label: "Novidades",
        action: () => {
          this._filtered = this._allProducts.filter((p) => p.badge === "Novo");
          this._render();
        },
      },
    ];

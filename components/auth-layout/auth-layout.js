class AuthLayout extends HTMLElement {
  connectedCallback() {
    ...
    if (localStorage.getItem("isLoggedIn") === "true") {
      ...
      window.location.href = redirect === "checkout" ? checkoutHref : homeHref;
      return;
    }
  ...
  _bindCep() {
    ...
    const buscar = async () => {
      ...
      if (!cep || cep.length !== 8) {
        this._setError(
          cepEl,
          "Digite um CEP válido com 8 dígitos (00000-000).",
        );
        return;
      }
      ...
      try {
        ...
        this._setError(
          form?.querySelector('[data-error="cep"]'),
  ...
  _bindRegisterSubmit() {
    ...
    form?.addEventListener("submit", (e) => {
      ...
      const name = val("name");
      if (!name) {
        this._setError(err("name"), "Nome é obrigatório.");
        hasError = true;
      }
      ...
      const confirm =
        form.querySelector('input[name="password-confirm"]')?.value ?? "";
      const cep = val("cep");

      const emailRegex = /^[^\s@]+@[^\s@]+\.[^\s@]+$/;
      if (!email) {
        this._setError(err("email"), "E-mail é obrigatório.");
        hasError = true;
      ...
      if (!password) {
        this._setError(err("password"), "Senha é obrigatória.");
        hasError = true;
      ...
      if (!confirm) {
        this._setError(err("password-confirm"), "Confirme sua senha.");
        hasError = true;
      } else if (password !== confirm) {
        this._setError(err("password-confirm"), "As senhas devem ser iguais.");
        hasError = true;
      }

      const cepDigits = cep.replace(/\D/g, "");
      if (!cep) {
        this._setError(err("cep"), "CEP é obrigatório.");
        hasError = true;
      } else if (cepDigits.length !== 8) {
        this._setError(err("cep"), "CEP deve ter 8 dígitos (00000-000).");
        hasError = true;
      }
  ...
  _bindLoginSubmit() {
    ...
    form?.addEventListener("submit", (e) => {
      ...
      if (!email) {
        ...
        this._setError(err("email"), "E-mail inválido.");
        hasError = true;
      }
      ...
      if (user) {
        ...
        window.location.href =
          redirect === "checkout" ? checkoutHref : homeHref;
      } else {
